...
async function unsubscribeUser(req, res, next) {
	try {
		...
		req.flash('success', 'You successfully unsubscribed from DevAlert NewsLetter');
		res.redirect('/');
	} catch (err) {
		console.error(err);
		next(err);
...
async function sendMail(req, res, next) {
	try {
		...
		const filename = path.normalize(path.join(__dirname, '../email-templates/welcome.hbs'));
		const html = fs
...
async function sendMailForRemoteJob() {
	try {
		...
		const file = fs.readFileSync(path.join(__dirname, '../email-templates/remote_job.hbs')).toString();
...
async function sendContactAlert(req, res, next) {
	try {
		...
		const filename = path.normalize(path.join(__dirname, '../email-templates/contact.hbs'));
		const html = fs
			.readFileSync(filename)
			.toString()
			.replace(/{{name}}/, name);
		...
		req.flash('success', 'Your message was sent. Our support would reply within 24 hours.');
...
async function sendMailForRemoteJobWeekly() {
	try {
		...
		var foundJobs = await User.find({ created_date: { $gt: date } });

		sendWeeklyEmail.sendWeeklyEmail(foundJobs);
	} catch (error) {
		console.log(error);
	}
}
// fire the email automatically by uncomment the function below
